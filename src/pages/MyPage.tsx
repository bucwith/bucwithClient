...
const MyPage = () => {
  ...
  const [interestCategoryModal, setInterestCategoryModal] =
    React.useState(false);
  const [nickname, setNickname] = React.useState("");
  const [profileIcon, setProfileIcon] = React.useState<string[]>([]);
  const [selectCategoryList, setSelectCategoryList] = React.useState<string[]>([
    "#영화",
    "#여행",
    "#대학합격",
    "#미용/뷰티",
  ...
  const {} = useQuery(["getUserData"], getUserData, {
    onSuccess(data) {
      ...
      setProfileIcon([data.iconCode, ICON_COLOR.bg_color[data.bgColor]]);
    },
  });
  ...
  return (
    <div>
      <VerticalCentered gap="20px">
        ...
        <ProfileIcon
          type="button"
          onClick={() => setIconSelectModal(true)}
          color={profileIcon[1]}
          icon={ICON_CODE[profileIcon[0]]}
        />
        <ProfileName type="button" onClick={() => setNicknameModal(true)}>
          {nickname}
        </ProfileName>
        <MypageSectionWrapper>
          <MypageSectionTitle
            type="button"
            onClick={() => setInterestCategoryModal(true)}
            icon={heart}
          >
            관심 카테고리
          </MypageSectionTitle>
        ...
        <MypageSectionWrapper>
          <MypageSectionTitle as="h2" icon={bell}>
            알림 설정
          </MypageSectionTitle>
            ...
            {dataTextList.map((v, index) => {
              return (
                <ToggleBtnWrapper
                  ...
                  as="li"
                  data-text={v}
                  className={index === 2 ? "isOn" : ""}
                >
                  <ToggleBtn
                    className={index === 2 ? "isOn" : ""}
                    type="button"
                    onClick={(e) => {
                      (e.target as Element).classList.toggle("isOn");
                      (
                        (e.target as Element).parentNode as Element
                      ).classList.toggle("isOn");
                    }}
                  />
                </ToggleBtnWrapper>
              );
      ...
      {iconSelectModal && (
        <IconSelectModal
          setIconSelectModal={setIconSelectModal}
      ...
      {interestCategoryModal && (
        <SelectCategoryModal
          setInterestCategoryModal={setInterestCategoryModal}
          interestCategoryModal={interestCategoryModal}
          selectCategoryList={selectCategoryList}
          setSelectCategoryList={setSelectCategoryList}
        />
      )}
    </div>
  );
