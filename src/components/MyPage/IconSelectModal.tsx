...
import {
  ...
  Color,
  BtnWrapper,
} from "./Modal/IconSelectModalStyledComponent";
import { ICON_COLOR, ICON_CODE, MODAL_BGCOLOR } from "../../constant";
...
const IconSelectModal = ({
  ...
  const [selectIconColor, setSelectIconColor] = React.useState(
    profileIcon[0].slice(-1) === "B"
      ? 8
      : profileIcon[0].slice(-1) === "Y"
      ? 4
      : 0
  );
  const [selectIcon, setSelectIcon] = React.useState(
    Object.keys(ICON_CODE).indexOf(profileIcon[0])
  );
        ...
        <Container>
          <FlexBox as="form" gap="30px">
            ...
            <FlexBox as="ul" gap="10px" direction="row">
              ...
              <Icon
                ...
                iconColor={
                  Object.values(ICON_CODE)[
                    Math.abs(
                      selectIcon % 4 < 0 ? (selectIcon % 4) + 4 : selectIcon % 4
                    ) + selectIconColor
                  ]
                }
              />
              <BtnWrapper>
                <IconMoveBtn
                  ...
                  onClick={() => {
                    setSelectIcon(
                      selectIcon + 1 > selectIconColor + 3
                        ? selectIcon - 3
                        : selectIcon + 1
                    );
                  }}
                ...
                {ICON_COLOR.icon_color.map((v, index) => {
                  return (
                    <Color
                      key={index}
                      onClick={() => {
                        setSelectIconColor(index * 4);
                        setSelectIcon(selectIcon - selectIconColor + index * 4);
                      }}
                      color={v}
                      select={selectIconColor === 0 + index * 4}
                    />
                  );
                })}
              </FlexBox>
            ...
            <PrimaryButton
              ...
              onClick={() => {
                putUserIcon(
                  Object.keys(ICON_CODE)[selectIcon],
                  Object.keys(ICON_COLOR.bg_color).find(
                    (key) => ICON_COLOR.bg_color[key] === selectColor
                  )
                );
                setIconSelectModal(false);
              }}
